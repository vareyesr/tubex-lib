...
namespace tubex
{
  CtcCidGuess::CtcCidGuess(const TFnc& fnc, double prec): fnc(fnc), prec(prec)
	{
		//assert(prec >= 0);
		set_prec(0.05);
	}

	bool CtcCidGuess::contract(std::vector<Slice*> x_slice, std::vector<Slice*> v_slice, TimePropag t_propa)
	{
	...
	void CtcCidGuess::ctc_fwd(Slice &x, Slice &v, std::vector<Slice*> x_slice, std::vector<Slice*> v_slice, int pos)
	{
		/*envelope*/
	...
	double CtcCidGuess::get_prec()
	{
		return this->prec;
	}

	bool CtcCidGuess::get_max_it()
	{
		return this->max_it;
	}

	int CtcCidGuess::get_propagation_engine(){
		return this->engine;
	}

	int CtcCidGuess::get_s_corn(){
		return this->s_strategy;
	}

	int CtcCidGuess::get_dpolicy(){
		return this->d_policy;
	}

	void CtcCidGuess::set_prec(double prec){
		this->prec = prec;
	}

	void CtcCidGuess::set_max_it(bool max_it){
		this->max_it = max_it;
	}

	void CtcCidGuess::set_propagation_engine(int engine){
		this->engine = engine;
	}

	void CtcCidGuess::set_s_corn(int s_strategy){
	...
	std::vector<std::vector<double>> CtcCidGuess::cart_product (const std::vector<std::vector<double>>& v) {
	...
	void CtcCidGuess::create_corners(std::vector<Slice> x_slices, std::vector< std::vector<double> > & points, TimePropag t_propa){

		std::vector< std::vector<double> > aux_points;
		//for each dimension, obtain the corresponding corners
		if (t_propa & TimePropag::FORWARD){
			for (unsigned int i=0 ; i < x_slices.size() ; i++){
				vector<double> aux;
				aux.clear();
	...
	void CtcCidGuess::var3Bcheck(ibex::Interval remove_bound ,int bound, int pos ,std::vector<Slice*> & x_slice, std::vector<Slice*> v_slice, TimePropag t_propa)
	{
...
//			if (max_iterations>=50) set_max_it(true);  // max iterations reached
		...
		if (get_dpolicy() != 0){
			for (int k = 0 ;  k < 1 ; k++){
				...
				success = false;
				for (unsigned int i = 0 ;  i < x_slice_bounds.size() ; i++){
					double sx;
					CtcDeriv ctc_deriv;
					int max_iterations=0;
	//				/*without polygons*/
	...
	void CtcCidGuess::AtomicPropagationEngine(std::vector<Slice> & x_slice, std::vector<Slice> & v_slice, TimePropag t_propa){
		...
		for (unsigned int i = 0 ; i < x_slice.size() ;i++){
			...
			create_bounds(x_slice[i],x_subslices, t_propa);
  ...
  void CtcCidGuess::FullPropagationEngine(std::vector<Slice> & x_slice, std::vector<Slice> & v_slice, TimePropag t_propa){
			...
			for (unsigned int i = 0 ; i < x_slice.size() ; i++){
				...
				for (int k = 0 ; k < nb_iterations ; k++){
					...
					if (get_s_corn() == 0){
						...
						if (t_propa & TimePropag::FORWARD)
							for (unsigned int tt = 0 ; tt < points[k].size() ; tt++)
